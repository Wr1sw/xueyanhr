        ...
        methods: {
            deleteRole(role) {
                this.$confirm('此操作将永久删除【' + role.nameZh + '】角色, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/system/cfg/permiss/role/" + role.id).then(resp => {
                        if (resp) {
                            this.initRoles();
                        }
                    })
            ...
            doAddRole() {
                if (this.role.name && this.role.nameZh) {
                    ...
                    this.postRequest("/system/cfg/permiss/role", this.role).then(resp => {
            ...
            doUpdate(rid, index) {
                ...
                let url = '/system/cfg/permiss/?rid=' + rid;
            ...
            initSelectedMenus(rid) {
                this.getRequest("/system/cfg/permiss/mids/" + rid).then(resp => {
                    if (resp) {
            ...
            initAllMenus() {
                this.getRequest("/system/cfg/permiss/menus").then(resp => {
                    if (resp) {
                        this.allmenus = resp;
                    }
                })
            ...
            initRoles() {
                this.loading = true;
                this.getRequest("/system/cfg/permiss/").then(resp => {
                    this.loading = false;
                    if (resp) {
                        this.roles = resp;
                    }
